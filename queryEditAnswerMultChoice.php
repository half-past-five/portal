...
<head>
    ...
    <title>Edit Mult Choice Answer</title>
    <meta charset="utf-8">
...
<body class="img js-fullheight" style="background-image: url(https://images.saymedia-content.com/.image/t_share/MTc4NzM1OTc4MzE0MzQzOTM1/how-to-create-cool-website-backgrounds-the-ultimate-guide.png);">
    ...
    $tsql = "{call EditAnswerMultChoice(?,?,?,?)} ";
    $UserID = $_SESSION["User ID"];
    echo "Executing query: " . $tsql . ") with parameter " . $UserID . $_POST["question_id"] . $_POST["answer"] . $_POST["new_answer"] .  "<br/>";

    $params = array(
        array($UserID, SQLSRV_PARAM_IN),
        array($_POST["question_id"], SQLSRV_PARAM_IN),
        array($_POST["answer"], SQLSRV_PARAM_IN),
        array($_POST["new_answer"], SQLSRV_PARAM_IN)
    );
